...
from bot import LOGGER, bot, user_data, config_dict, bot_name
...
async def list_buttons(user_id, isRecursive=True, user_token=False):
    buttons = ButtonMaker()
    buttons.ibutton("Folders", f"list_types {user_id} folders {isRecursive} {user_token}")
    ...
    buttons.ibutton(f"User Token: {user_token}",
...
async def _list_drive(key, message, item_type, isRecursive, user_token, user_id):
    ...
    if user_token:
        ...
        target_id = ''
    telegraph_content, contents_no = await sync_to_async(gdSearch(isRecursive=isRecursive, itemType=item_type).drive_list, key, target_id, user_id)
    if telegraph_content:
        ...
        if config_dict['BOT_PM'] and message.chat.type != message.chat.type.PRIVATE:
            ...
            await send_to_chat(user_id, msg, button, photo=True)
            await editMessage(message, ibmsg, pmbutton)
        else:
            await editMessage(message, msg, button)
    else:
        await editMessage(message, f'Hey.\n\nNo result found for <i>{key}</i>')
...
bot.add_handler(MessageHandler(gdrive_search, filters=command(
